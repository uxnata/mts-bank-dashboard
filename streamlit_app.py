...
if not df.empty:
    ...
    st.info("""
    ### Проверьте:
    1. ✅ Добавлены ли секреты SUPABASE_URL и SUPABASE_KEY
    2. ✅ Правильные ли ключи доступа
    3. ✅ Есть ли таблица reviews в Supabase
    4. ✅ Есть ли данные в таблице
    
    ### Для добавления секретов:
    Settings → Secrets → Вставьте ключи → Save
    """)

# Финальный автоматический рефреш (опционально)
if 'auto_refresh' not in st.session_state:
    st.session_state.auto_refresh = False

if st.sidebar.checkbox("🔄 Автообновление каждые 60 сек", value=False):
    import time
    time.sleep(60)
    st.rerun()
